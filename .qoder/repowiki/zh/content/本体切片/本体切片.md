...
- **Producer**（`ontology_extraction` skill）：
  - [src/OpenClaw.Plugins.EmploymentCoachWorkflow/skills/ontology_extraction/SKILL.md](file://src/OpenClaw.Plugins.EmploymentCoachWorkflow/skills/ontology_extraction/SKILL.md)
  - 模板目录：`templates/TEMPLATE.json` + `TEMPLATE.schema.json`、`PROJECTION_TEMPLATE.json` + `.schema.json`
  - 样例目录：`examples/{ready,warning,invalid}`
...
数字员工模板 ZIP 包（参见 [数字员工端点](../网关服务/HTTP端点/数字员工端点.md)）通过 [DigitalEmployeeEndpoints.cs](file://src/OpenClaw.Gateway/Endpoints/DigitalEmployeeEndpoints.cs) 上传时，**只接受顶层 `.md` 形式的 ontology 文件**：
...
| **SkillLoader 自动发现 `contracts/projections/**/contract-index.json`** | ⚠️ **设计稿描述但本仓未落地** | [SkillLoader.cs](file://src/OpenClaw.Core/Skills/SkillLoader.cs) 中无 `projection`/`contract` 关键字 |
| Discovery diagnostics（none/bound/partial/...）| ⚠️ 同上未落地 | — |

> 即：当前仓库的 `ProjectionContracts` 必须由其它路径填充到 `SkillDefinition`（例如配置或人工接线），而不是 `SkillLoader.LoadAll` 自动扫描磁盘。这是接下来重要的工程缺口之一。

---

## 扩展阅读

- 上一层导航：[网关服务 / HTTP 端点 / 数字员工端点](../网关服务/HTTP端点/数字员工端点.md)
- 相关主题：[开发指南](../开发指南/开发指南.md)（§ 校验脚本部分提到 ontology slice/projection）
