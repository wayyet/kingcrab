...
本目录除主文档外，已从 `E:\Download\本体\kingcrab\网关服务` 合并入仓 4 个子主题目录与 2 篇顶层细化文档，按「子主题入口 → 细分专题」两层组织。所有子文档均遵循 RepoWiki 标准结构：`cite` / 项目结构 / 核心组件 / 架构总览 / 详细组件分析 / 依赖关系分析 / 性能考量 / 故障排查指南 / 结论 / 附录。

### 顶层细化文档（同目录）

| 文档 | 核心覆盖 | 关键源码 |
|------|----------|----------|
| [网关架构设计.md](./网关架构设计.md) | 启动引导、服务组合、端点映射、中间件管线、运行时初始化的分层组织与职责边界 | `Program.cs` / `GatewayStartupContext.cs` / `MessagePipeline.cs` |
| [健康检查与监控.md](./健康检查与监控.md) | 插件健康检查、运行时指标、Provider/Tool 使用追踪、Telemetry 接线与可观测性 | `PluginHealthService.cs` / `RuntimeMetrics.cs` / `Telemetry.cs` / `ProviderUsageTracker.cs` / `ToolUsageTracker.cs` |

### 子主题目录

| 子主题 | 入口文档 | 包含细分专题 | 关键源码 |
|--------|----------|--------------|----------|
| HTTP API 端点 | [HTTP API 端点/HTTP API 端点.md](./HTTP%20API%20端点/HTTP%20API%20端点.md) | [OpenAI 兼容 API](./HTTP%20API%20端点/OpenAI%20兼容%20API.md) / [管理员管理端点](./HTTP%20API%20端点/管理员管理端点.md) / [集成 API 端点](./HTTP%20API%20端点/集成%20API%20端点.md) / [诊断监控端点](./HTTP%20API%20端点/诊断监控端点.md) | `Endpoints/AdminEndpoints*.cs` / `Endpoints/IntegrationEndpoints.cs` / `Endpoints/DiagnosticsEndpoints.cs` / OpenAI 兼容路由 |
| WebSocket 通信 | [WebSocket 通信/WebSocket 通信.md](./WebSocket%20通信/WebSocket%20通信.md) | [连接管理](./WebSocket%20通信/连接管理.md) / [消息处理](./WebSocket%20通信/消息处理.md) / [实时会话](./WebSocket%20通信/实时会话.md) / [安全认证](./WebSocket%20通信/安全认证.md) | `WebSocketChannel.cs` / `OpenClawWebSocketClient.cs` |
| 中间件管道 | [中间件管道/中间件管道.md](./中间件管道/中间件管道.md) | [消息中间件](./中间件管道/消息中间件.md) / [速率限制中间件](./中间件管道/速率限制中间件.md) / [令牌预算中间件](./中间件管道/令牌预算中间件.md) / [工作者集成](./中间件管道/工作者集成.md) / [管道配置](./中间件管道/管道配置.md) | `MessagePipeline.cs` / `RateLimitMiddleware.cs` / `TokenBudgetMiddleware.cs` / `ActorRateLimitService.cs` |
| 认证与授权 | [认证与授权/认证与授权.md](./认证与授权/认证与授权.md) | [Bearer Token 认证](./认证与授权/Bearer%20Token%20认证.md) / [访问控制与权限管理](./认证与授权/访问控制与权限管理.md) / [安全策略与配置/](./认证与授权/安全策略与配置/安全策略与配置.md)（含严格公共绑定/公共绑定加固/秘密管理/通信渠道安全 4 篇）/ [渠道 Webhook 认证/](./认证与授权/渠道%20Webhook%20认证/渠道%20Webhook%20认证.md)（含 Bot Framework / Twilio SMS / 最佳实践 3 篇） | `GatewaySecurity.cs` / `Endpoints/AdminEndpoints.Auth.cs` / `*WebhookHandler.cs`（Discord/Slack/Teams/Telegram/Twilio/WhatsApp） |

### 与现有平铺文档的关系

本目录原有 5 篇平铺整合文档（`启动流程和配置.md` / `中间件管道.md` / `端点路由和API.md` / `安全策略和认证.md` / `运行时管理和生命周期.md`）保留为高层视角入口，与上述子目录形成「平铺速览 ⇄ 嵌套深挖」双层互补：

- **`端点路由和API.md`** ↔ 深入 → `HTTP API 端点/`
- **`中间件管道.md`** ↔ 深入 → `中间件管道/`
- **`安全策略和认证.md`** ↔ 深入 → `认证与授权/`
- **`启动流程和配置.md`** / **`运行时管理和生命周期.md`** ↔ 深入 → `网关架构设计.md`

如对某一主题信息出现冲突，**以子目录中的细分专题文档为准**（更新时间更近且粒度更细）；高层信息以现有平铺文档为准。
