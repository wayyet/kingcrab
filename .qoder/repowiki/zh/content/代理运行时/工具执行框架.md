...
**更新** 本版本修复了关键数据竞争问题，增强了线程安全性，包括对 ToolDeclarations 属性、GetToolDeclarations 方法、SupportsStreaming 方法和内部工具查找过程的保护。
...
- 流式工具接口 IStreamingTool：可选接口，支持增量输出的流式会话。
...
**图表来源**
- [OpenClawToolExecutor.cs:156-660](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L156-L660)
...
**更新** 该组件现已修复关键数据竞争问题，通过以下机制确保线程安全：

- **全局锁保护**：使用 `_toolsMutationLock` 对所有工具访问进行同步
- **原子性操作**：ReplaceMcpTools 方法现在是原子性的，确保热重载时的一致性
- **快照机制**：在执行过程中创建工具名称的快照，避免并发修改导致的问题
- **属性访问保护**：ToolDeclarations、GetToolDeclarations、SupportsStreaming 方法均受锁保护
...
+ExecuteStreamingAsync(argsJson,ct)
}
...
OpenClawToolExecutor --> IStreamingTool : "可选依赖"
...
**章节来源**
- [OpenClawToolExecutor.cs:102-154](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L102-L154)
- [OpenClawToolExecutor.cs:156-660](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L156-L660)
...
- [OpenClawToolExecutor.cs:156-171](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L156-L171)
...
**章节来源**
- [OpenClawToolExecutor.cs:163-660](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L163-L660)
...
**章节来源**
- [OpenClawToolExecutor.cs:586-607](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L586-L607)
- [OpenClawToolExecutor.cs:733-766](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L733-L766)
- [OpenClawToolExecutor.cs:768-798](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L768-L798)

### 工具开发指南
- 实现 ITool：提供 Name、Description、ParameterSchema 与 ExecuteAsync
- 可选实现 IStreamingTool：支持增量回调的流式工具
...
**章节来源**
- [OpenClawToolExecutor.cs:566-572](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L566-L572)
...
Executor --> Istream["IStreamingTool"]
...
- **数据竞争问题**：如遇到工具列表不一致或并发访问异常，确认已使用修复后的线程安全版本

**章节来源**
- [OpenClawToolExecutor.cs:197-208](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L197-L208)
- [OpenClawToolExecutor.cs:305-335](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L305-L335)
- [OpenClawToolExecutor.cs:513-560](file://src/OpenClaw.Agent/OpenClawToolExecutor.cs#L513-L560)
- [ToolSandboxException.cs](file://src/OpenClaw.Core/Abstractions/ToolSandboxException.cs)

## 结论
工具执行框架通过清晰的分层与接口抽象，实现了从工具发现、注册、治理授权、权限控制、超时与错误处理到审计与指标的完整闭环。配合多后端与沙箱执行能力，既满足本地开发的灵活性，又兼顾生产环境的安全与隔离需求。

**更新** 最新版本已修复关键数据竞争问题，通过全局锁保护、原子性操作和快照机制确保了高并发场景下的稳定性和一致性。建议在实际部署中结合业务场景优化超时与回退策略，并充分利用审计与治理记录进行持续改进。
