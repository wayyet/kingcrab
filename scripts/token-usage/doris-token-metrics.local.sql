-- Local integration-test variant of doris-token-metrics.sql.
-- Differences from the prod script:
--   * replication_num = 1 (single-BE docker deployment, see setting_Install/kafka-doris-deploy)
--   * kafka_broker_list points at the in-network listener kafka-doris-kafka:29092
--     (the host-facing listener advertises localhost:9092, unreachable from Doris containers)
--   * materialized view refresh interval shortened to 1 minute for faster test feedback

CREATE DATABASE IF NOT EXISTS token_metrics;

CREATE TABLE IF NOT EXISTS token_metrics.session_token_events (
...
FROM KAFKA (
    "kafka_broker_list"              = "kafka-doris-kafka:29092",
    "kafka_topic"                    = "session-token-metrics",
    "property.group.id"              = "doris-token-loader",
    "property.kafka_default_offsets" = "OFFSET_END"
);

-- Doris (unlike StarRocks) async-MV refresh syntax: REFRESH AUTO ON SCHEDULE EVERY n unit
