...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Root-level wrapper for runtime skill projection document validation."
    )
    parser.add_argument("paths", nargs="*", help="Runtime *.projection.json paths to validate.")
    parser.add_argument("--schema-path", dest="schema_path", help="Optional schema path.")
    args = parser.parse_args()

    repo_root = Path(__file__).resolve().parent.parent
    validator_path = repo_root / "src" / "OpenClaw.Plugins.EmploymentCoachWorkflow" / "skills" / "ontology_extraction" / "scripts" / "validate-projection.py"
    ...
    input_paths = args.paths or [str(default_input_path)]
    for input_path in input_paths:
        invoke_args.append(str(resolve_absolute_path(input_path, current_base)))

    schema_path = resolve_absolute_path(args.schema_path, current_base) if args.schema_path else default_schema_path.resolve()
    invoke_args.extend(["--schema-path", str(schema_path)])
