# 从 ontology_extraction 到具体业务 Skill 的构建方法

本文整理一条在当前仓库内已经成型的实践路径：

1. 从 `ontology_extraction` skill 出发，为当前业务问题构建一个最小可验证的本体切片。
...
- [docs/ontology_extraction-first-skill-practical-template.md](docs/ontology_extraction-first-skill-practical-template.md)：从零为一个新业务 Skill 落第一份 slice、第一个任务域和第一个 projection 的实操模板。
...
换句话说，`ontology_extraction` 负责“把业务知识整理成可验证语义输入，并生成可交付的 projection 草案/产物”。

### 2.2 consumer skill：具体业务 Skill

consumer 侧在当前仓库里的实例如下：

- [src/OpenClaw.Gateway/skills/software-developer/contracts/projections/ontology_extraction/README.md](src/OpenClaw.Gateway/skills/software-developer/contracts/projections/ontology_extraction/README.md)
- [src/OpenClaw.Gateway/skills/software-developer/contracts/projections/ontology_extraction/contract-index.json](src/OpenClaw.Gateway/skills/software-developer/contracts/projections/ontology_extraction/contract-index.json)
...
从 `ontology_extraction` 到业务 Skill 的落地过程，可以拆成六步。
...
这一步的目标是收缩范围。`ontology_extraction` 的原则是只保留当前任务所需的最小语义闭包，而不是导出整份领域本体。
...
slice 只负责“语义闭包”，并不天然等于业务 Skill 的最终使用形态。真正给业务 Skill 用时，需要由 `ontology_extraction` 按交付视图和映射规范把它显式改写成 projection。常见交付视图例如：
...
不要直接从 consumer 侧写 projection。先用 `ontology_extraction` 把语义闭包切出来，再由 producer 侧按交付视图和映射规范生成 projection，最后再交给 consumer 承载与消费。
...
#### 第一步：先用 `ontology_extraction` 把 slice 收缩并校验通过
...
- 用 `TEMPLATE.schema.json` 对应校验，或按所在层级选择脚本入口：在 `ontology_extraction` 技能根目录使用 `validate-slice`，在仓库根目录使用 `validate-ontology-slice`。
...
#### 第三步：让 `ontology_extraction` 按映射规范填充 projection 模板
...
此时的落点不应再是 `ontology_extraction` 自己的模板目录，而应进入 consumer skill 的正式 contract 目录，例如：
...
1. 在 `ontology_extraction` 中先产出一份通过校验的 JSON slice；如需评审或人读说明，可再补一份配套 Markdown。
...
从 `ontology_extraction` 构建业务本体切片，再把本体切片应用到具体业务 Skill，本质上是一条三层转换链：
...
到这个阶段，`ontology_extraction` 不再只是一个“建模说明 skill”，而是 producer 侧的语义治理入口；而具体业务 Skill 也不再只是“读文档猜规则”，而是通过 projection contracts 在运行时正式消费这些语义约束。
