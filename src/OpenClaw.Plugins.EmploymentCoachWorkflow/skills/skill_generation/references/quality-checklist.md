...
- [ ] Input type is classified as handoff todo, conversation, upload, or mixed.
- [ ] In handoff mode, every dispatched todo id appears in `todo_results` with `success`, `skipped`, or `failed`.
- [ ] In handoff mode, `skill_name`, `skill_description`, `trigger`, `expected_output`, `source`, and `acceptance` are preserved in source or extraction notes.
...
- [ ] If a READY projection contract is generated, `contracts/projections/ontology_extraction/contract-index.json` exists.
...
- [ ] `todo_results` maps each handoff todo id to artifacts, acceptance result, or readable failure reason.
