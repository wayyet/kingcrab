...
- For human review, read `contracts/projections/ontology_extraction/contract-index.json` first, then the selected topic's `README.md` and `REVIEW.md`, and then the chosen `*.projection.json` file.
...
- 不把 `skill_generation` 自身注册为 projection consumer；projection consumer 结构只写入生成出来的业务 skill。
...
- `../ontology_extraction/references/CONSUMER_PROJECTION_LAYOUT_GUIDE.md`：consumer projection 目录命名规范。
