...
`external-config` 是雇佣教练流程阶段三的下游执行型 skill。它接收 `employment-coach-conversation` 产出的 external 系统 todo，把“要接哪个外部系统、做什么能力、需要哪些字段和认证形式”落成沙箱内 `external/` 目录下的配置草案。

它不做对话引导，不收集真实凭据，不修改系统 todo 状态，也不直接调用外部系统。
...
2. 系统层把本次 `todos` 对应的 external 系统 todo 传给 `external-config`。
3. `external-config` 校验字段、扫描疑似凭据、生成 capability 与 system 配置草案。
4. 如系统层提供安全表单凭据上下文，只绑定 `credentialSlot` / `secretRef`，不把真实值写入普通产物。
5. 产物写入沙箱 `external/`。
6. `external-config` 返回 `dispatch_callback`，包含 `user_summary`、artifact 相对路径和逐 todo 的 `todo_results`。
...
| --- | --- | --- |
| `employment-coach-conversation` | 引导用户，生成 external 系统 todo，发 dispatch | 否 |
| `external-config` | 把 external 系统 todo 落成配置草案 | 是 |
