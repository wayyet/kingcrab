...
3. **Handoff 先行**：进入沙箱会话后，第一轮动作就是调用 Handoff tool 新建或更新阶段 1 的 Handoff todo，先把资料阶段的待收集 / 待处理事项落到交接工单里，再给用户一句反馈；后续只要用户给出可交给下游处理的资料、技能或外部能力信息，也必须先调用 Handoff tool 新建或更新 Handoff todo，再给用户一句反馈；不能只在对话里复述、分析或生成结果
