...
internal sealed class CronTool : IToolWithContext
{
    ...
    public string Description => "Manage scheduled cron jobs. List configured jobs, get details, trigger immediate execution, create new jobs, or delete existing jobs.";
    public string ParameterSchema => """
    {
      "type":"object",
      "properties":{
        "action":{"type":"string","enum":["list","get","run","create","delete","history"],"description":"Action to perform"},
        "name":{"type":"string","description":"Job name (required for get/run/delete/history; used as display name for create)"},
        "schedule":{"type":"string","description":"Cron expression or shorthand like @daily, @hourly (required for create unless run_at is set)"},
        "prompt":{"type":"string","description":"Prompt text for the job (required for create)"},
        "timezone":{"type":"string","description":"IANA timezone, e.g. America/New_York (create)"},
        "model_id":{"type":"string","description":"LLM model override for this job (create)"},
        ...
        "channel_id":{"type":"string","description":"Delivery channel ID (create)"},
        "recipient_id":{"type":"string","description":"Delivery recipient ID (create)"},
        "run_on_startup":{"type":"boolean","description":"Run immediately on gateway startup (create)"},
    ...
    public async ValueTask<string> ExecuteAsync(string argumentsJson, ToolExecutionContext context, CancellationToken ct)
    {
        ...
        return action switch
        {
            ...
            _ => $"Error: Unknown action '{action}'. Use list, get, run, create, delete, or history."
        };
    }
    ...
    private async Task<string> CreateJobAsync(JsonElement root, ToolExecutionContext context, CancellationToken ct)
    {
        ...
        var definition = new AutomationDefinition
        {
            ...
            DeliveryChannelId = GetString(root, "channel_id") ?? context.Session.ChannelId,
            DeliveryRecipientId = GetString(root, "recipient_id") ?? context.Session.SenderId,
            IsDraft = false,
            Source = "agent",
            RunAt = runAt,
