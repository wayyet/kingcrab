...
- Keep `## Projection Contracts` only if the new skill directly consumes `ncrew-ontology` projection contracts.
...
- Keep the shared references to `../ncrew-ontology/templates/CONSUMER_SKILL_PROJECTION_SECTION.md`, `../ncrew-ontology/references/PROJECTION_CONSUMPTION_GUIDE.md`, and `../ncrew-ontology/references/CONSUMER_PROJECTION_LAYOUT_GUIDE.md` if the new skill consumes projection contracts.
- Delete unused references if the skill is not a projection consumer.
- If the new skill will store local bound contracts, create the matching `contracts/projections/ncrew-ontology/<domain-slug>/` layout.
